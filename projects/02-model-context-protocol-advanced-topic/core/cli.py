...
class CliApp:
    ...
    async def initialize(self):
        # Startup fetches resources/prompts once so autocomplete has fresh MCP data.
        logger.info("Initializing CLI resources and prompts")
    ...
    async def refresh_resources(self):
        try:
            ...
            # Count logs are a quick sanity check that the MCP server responded.
            logger.info("Loaded %d document resources", len(self.resources))
        except Exception as e:
            # Use exception logging for developers, while print keeps the CLI friendly.
            logger.exception("Error refreshing resources")
    ...
    async def refresh_prompts(self):
        try:
            ...
            # Prompt count tells us whether slash-command discovery is working.
            logger.info("Loaded %d prompts", len(self.prompts))
        except Exception as e:
            # Keep both the traceback in logs and a short message in the terminal.
            logger.exception("Error refreshing prompts")
