...
class ToolManager:
    ...
    async def get_all_tools(cls, clients: dict[str, MCPClient]) -> list[Tool]:
        ...
        # This confirms the client collected the tool definitions sent to Claude.
        logger.info(
            "Discovered %d total tools across %d MCP clients",
            len(tools),
            len(clients),
        )
        return tools
    ...
    async def _find_client_with_tool(
        ...
        for client in clients:
            ...
            if tool:
                # Debug level is enough here because this can happen often in tool loops.
                logger.debug("Found tool '%s' on client '%s'", tool_name, client.name)
                return client
        logger.warning("Could not find MCP tool '%s'", tool_name)
        return None
    ...
    async def execute_tool_requests(
        ...
        for tool_request in tool_requests:
            ...
            # Log the model-requested tool call before selecting the MCP client.
            logger.info(
                "Executing tool request '%s' with id '%s'",
                tool_name,
                tool_use_id,
            )
            ...
            try:
                ...
                # logger.exception includes the traceback, which is better than print.
