...
This is Project 02 in the Anthropic learning journey repo. It builds on the **Introduction to Model Context Protocol** project and adds advanced application ergonomics such as logging and progress visibility around MCP client-server interactions.
...
The app prints lightweight progress updates to stderr while MCP servers connect,
resources load, and tools run. Structured logs also go to stderr, which keeps
the MCP server's stdio transport safe.
...
`MCP_LOG_LEVEL` defaults to `INFO`. Set `MCP_PROGRESS=0` to hide the user-facing
`[progress]` messages.
